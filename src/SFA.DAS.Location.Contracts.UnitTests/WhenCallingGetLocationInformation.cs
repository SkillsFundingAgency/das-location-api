...
public class WhenCallingGetLocationInformation
{
    ...
    )
    {
        ...
        mockLocationApiClient
            .Setup(client =>
                client.Get<GetLocationsListItem>(
                    It.Is<GetLocationsApiRequest>(c => c.GetUrl.Contains(locationName.Trim()) && c.GetUrl.Contains(authorityName.Trim()))))
        ...
        result.GeoPoint.Should().BeEquivalentTo(apiLocationResponse.Location.Coordinates);
        ...
        LocationLookupService service)
    {
        ...
        var lat = 0;
        var lon = 0;
        var encodedLocation = UrlEncoder.Default.Encode($"{locationName}, {locationName}");
        
        var request = new GetLocationsApiRequest(encodedLocation, authorityName);

        mockLocationApiClient
            .Setup(client =>
                client.Get<GetLocationsListItem>(
                    It.Is<GetLocationsApiRequest>(c => c.GetUrl == request.GetUrl)))
            .ReturnsAsync(apiLocationResponse);
        ...
        LocationLookupService service)
    {
        ...
        mockLocationApiClient.Verify(x => x.Get<GetLocationsListItem>(It.IsAny<GetLocationsApiRequest>()), Times.Never);
        ...
        LocationLookupService service)
    {
        ...
        mockLocationApiClient
            .Setup(client =>
                client.Get<GetLocationsListItem>(
                    It.Is<GetPostcodesOutcodeApiRequest>(c => c.GetUrl.Contains(outcode))))
            .ReturnsAsync(apiLocationResponse);
        ...
        LocationLookupService service)
    {
        ...
        ApiRequests.V2.GetPostcodesApiRequest? capturedRequest = null;
        mockLocationApiClient
            .Setup(x => x.Get<LookupPostcodeV2Response>(It.IsAny<ApiRequests.V2.GetPostcodesApiRequest>()))
            .Callback<IGetApiRequest>(x => capturedRequest = x as ApiRequests.V2.GetPostcodesApiRequest)
        ...
        mockLocationApiClient
            .Setup(client =>
                client.Get<GetLocationsListResponse>(
                    It.Is<GetSearchApiRequest>(c => c.GetUrl.Contains(location))))
        ...
        LocationLookupService service)
    {
        ...
        mockLocationApiClient
            .Setup(client =>
                client.Get<GetLocationsListItem>(
                    It.Is<GetPostcodesOutcodeApiRequest>(c => c.GetUrl.Contains(location))))
            .ReturnsAsync(new GetLocationsListItem
            {
                Location = null
            });
