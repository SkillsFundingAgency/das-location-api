...
public class SuggestedAddress
{
    ...
    public static SuggestedAddress From(DpaResultPlacesApiItem source)
    {
        // thoroughfare parts are joined with a comma but the bulding number will be prepended with a space
        var thoroughfare = string.Join(", ", (new [] { source.DependentThoroughfareName, source.ThoroughfareName }).Where(s => !string.IsNullOrEmpty(s)));
        ...
        return new SuggestedAddress
        {
            ...
            Latitude = source.Lat,
            Match = source.Match,
            Country = source.CountryCode switch
            {
                "E" => "England",
                "N" => "Northern Ireland",
                "S" => "Scotland",
